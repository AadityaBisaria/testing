...
Aryan Retail Library is a browser voice assistant that searches a small mock
catalog, rents available titles, receives returned books, and lists a member's
current loans. Its catalog is held in memory and resets when the server restarts.
...
3. **Configure environment variables**:
   ...
   The server needs `DEEPGRAM_API_KEY`, `DEEPGRAM_VOICE_ID`,
   `VERTEX_PROJECT_ID`, and `VERTEX_CREDENTIALS_PATH`. The last value is the
   path to a Google Cloud service-account JSON file; keep that file out of the
   repository. `VERTEX_LOCATION` defaults to `asia-south1` and `VERTEX_MODEL`
